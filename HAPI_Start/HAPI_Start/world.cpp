...
void world::LevelLoad()
{
	...
	// MAIN ENTITIES
	// player entity
	Player* newPlayer = new Player(1000, 500, 100);
	entityVec.push_back(newPlayer);

	// enemy entity(ies)
	Enemy* newEnemy = new Enemy(200, 100, 100);
			...
			for (size_t i = 0; i < entityVec.size(); i++)
			{
				for (size_t j = i + 1; j < entityVec.size(); j++)
				{
					...
					if (entityVec[i]->IsEnemyOf(entityVec[j]))
					{
						...
						auto entity1 = entityVec[i];
						auto entity2 = entityVec[j];
						Rectangle rect1 = w_graphics->GetSpriteRect(entity1->GetSpriteName(), 0, entity1);
						Rectangle rect2 = w_graphics->GetSpriteRect(entity2->GetSpriteName(), 0, entity2);
						// if the rectangles intersect, what to do
						 if(rect1.Intersects(rect2))
						 {
							 // set to 100 for testing
							 entity1->ReduceHealth(100);
							 entity2->ReduceHealth(100);

							 if (!entity1->IsAlive()) {
								 HAPI.UserMessage("Entity 1 Died", "Notification");
							 }
							 if (!entity2->IsAlive()) {
								 HAPI.UserMessage("Entity 2 Died", "Notification");
							 }
						 }
					}
				};
